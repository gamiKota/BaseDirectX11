...
void TitlePlayer::Update() {
	...
	if (m_time > 1.f && !m_isCreate) {
		...
		for (int i = 0; i < 5; i++) {
			...
			obj = new GameObject3D(E_MODEL_PLAYER, "Player", "Player");
	
			pos.x = (float)GetRandom(-3000, -2500);
			pos.z = (float)GetRandom(-3325, -4000);
			pos.y = -2937.f;
	
			GameObject::Instantiate(obj, pos, Quaternion::identity, float3() + 0.5f);
